...
def split_by_position(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    booms = natsorted(
        [x for x in unique_vals if re.match(r"^S[RL]\sBoom\s\d", x)]
    )
...
def add_channel_hookup(wb: Workbook, vw_export: pd.DataFrame, show_info: ShowData) -> None:
...
def add_instrument_schedule(wb: Workbook, vw_export: pd.DataFrame, show_info: ShowData) -> None:
    positions, position_names = split_by_position(generate_df.instrument_schedule(vw_export))
